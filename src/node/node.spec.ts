...
describe('>>> Node', () => {
  ...
  const neighborTop = mockNodeFactory(new Vector2D(0, 1))
  const neighborRight = mockNodeFactory(new Vector2D(1, 2))
  const neighborBottom = mockNodeFactory(new Vector2D(2, 1))
